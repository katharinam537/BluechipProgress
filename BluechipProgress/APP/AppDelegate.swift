...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        AppsFlyerLib.shared().appleAppID = "6689513597"
        ...
        // Generate and save UUID if it does not exist
        if UserDefaults.standard.string(forKey: "PlayerUUID") == nil {
            let uuid = UUID().uuidString
            UserDefaults.standard.set(uuid, forKey: "PlayerUUID")
            print("Generated new Player UUID: \(uuid)")
        }
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        // Configure Messaging with APNs token
        ...
        Messaging.messaging().token { token, error in
            if let error = error {
                ...
                if let playerUUID = self.getPlayerUUID() {
                    print("Player UUID: \(playerUUID)")
                    UserManager.shared.sendUserData(playerUUID: playerUUID, deviceId: token, isProduction: false)
                } else {
                    print("Player UUID is nil")
                }
            }
        }
    }
    ...
    func getPlayerUUID() -> String? {
        let uuid = UserDefaults.standard.string(forKey: "PlayerUUID")
        print("Retrieved Player UUID from UserDefaults: \(String(describing: uuid))")
        return uuid
    }
